    ...
    return userHiers.map((hier) => {
      ...
      for (const direction in realsnImplieds) {
        ...
        filteredRealNImplied[dir].reals = reals
          .filter((real) => resultsFilter(real, dir as Directions, oppDir, arrow))
      ...
      const getFieldInHier = (dir: Directions) => (hier[dir][0]
        ? hier[dir].join(', ')
        : `${hier[getOppDir(dir)].join(',')}${ARROW_DIRECTIONS[dir]}`);

      const squares = [ru, rs, rd, rn, rp, iu, is, id, iN, ip];

      if (enableAlphaSort) squares.forEach((sq) => sq.sort(this.sortItemsAlpha));
