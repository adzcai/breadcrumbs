...
export const forceDirectedG = (
  ...
  colourChangeInput.on('change', (el) => {
  ...
  const nameFromIndex = (d: { index: number }) => data.nodes.find((node) => node.id === d.index).name;

  const indexFromName = (name: string): number => data.nodes.find((node) => node.name === name).id;
  ...
  function linked(a: number, b: number) {
    ...
    const linkedArr = links.find(
      (link) => (link.source.index === a && link.target.index === b)
  ...
  node
    .on('mouseover', (event: MouseEvent, d: { index: number }) => {
      node
        .transition()
        .duration(150)
        .style('opacity', (o) => (linked(d.index, o.index) ? 1 : 0.2));
      link
        .transition()
        .duration(150)
        .style('opacity', (o) => (o.source.index === d.index || o.target.index === d.index
          ? 1
          : 0.2));
